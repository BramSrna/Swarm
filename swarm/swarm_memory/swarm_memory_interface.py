...
class SwarmMemoryInterface(object):
    ...
    def write_to_swarm_memory(self, key_to_write, value_to_write, data_type):
        ...
        self.executor_interface.send_propagation_message(MessageTypes.SWARM_MEMORY_OBJECT_LOCATION, {"OBJECT_ID": key_to_write, "LOCATION_ID": self.executor_interface.get_id(), "DATA_TYPE": data_type})
    ...
    def read_from_swarm_memory(self, key_to_read):
        ...
        if bot_with_obj == self.executor_interface.get_id():
            ...
            value = self.executor_interface.send_directed_message(bot_with_obj, MessageTypes.REQUEST_SWARM_MEMORY_READ, {"KEY_TO_READ": key_to_read}, True).get_message_payload()["OBJECT_VALUE"]
        else:
    ...
    def pop_from_swarm_memory(self, key_to_pop):
        ...
        if bot_with_obj == self.executor_interface.get_id():
            ...
            self.executor_interface.send_propagation_message(MessageTypes.DELETE_FROM_SWARM_MEMORY, {"KEY_TO_DELETE": key_to_pop})
        elif bot_with_obj is not None:
            self.local_swarm_memory.delete(key_to_pop)
            value = self.executor_interface.send_directed_message(bot_with_obj, MessageTypes.POP_FROM_SWARM_MEMORY, {"KEY_TO_POP": key_to_pop}, True).get_message_payload()["OBJECT_VALUE"]
            self.executor_interface.send_propagation_message(MessageTypes.DELETE_FROM_SWARM_MEMORY, {"KEY_TO_DELETE": key_to_pop})
        else:
            # else in this case means that the there is no key with the given value in the swarm memory
            pass
