...
class SwarmMemoryInterface(object):
    ...
    def write(self, path_to_write, value):
        if (not self.path_exists_in_memory(path_to_write)) or \
                (not isinstance(self.get_obj_at_path(path_to_write), LocalSwarmMemoryEntry)):
            self.initialize_path_in_memory(path_to_write)

        if not self.has_path_saved_locally(path_to_write):
            if self.local_memory_is_full():
                self.__make_space_in_memory()
            self.set_save_path_locally(path_to_write, True)

        self.get_obj_at_path(path_to_write).create_change_block(value)

        self.check_path_watchers(path_to_write)

    def read(self, path_to_read):
        if not self.path_exists_in_memory(path_to_read):
            return None

        return self.__unwrap(self.get_obj_at_path(path_to_read))
    ...
    def delete(self, path_to_delete):
        ...
        self.check_path_watchers(path_to_delete)

    def initialize_path_in_memory(self, path_to_create):
    ...
    def has_path_saved_locally(self, path_to_check):
        if not self.path_exists_in_memory(path_to_check):
            return False

        return self.get_obj_at_path(path_to_check).is_saved_locally()

    def local_memory_is_full(self):
    ...
    def __make_space_in_memory(self):
        ...
        self.executor_interface.send_propagation_message(
            ...
            {"PATH": lru_path, "BLOCKCHAIN": self.get_obj_at_path(lru_path).get_change_blocks()}
        )
        self.set_save_path_locally(lru_path, False)

    def set_save_path_locally(self, path, new_save_state):
        ...
        self.get_obj_at_path(path).set_saved_locally(new_save_state)
    ...
    def __unwrap(self, value_to_unwrap):
        if isinstance(value_to_unwrap, dict):
            ...
            if value_to_unwrap.is_saved_locally():
                self.__add_access(value_to_unwrap.get_path(), self.executor_interface.get_id())
            return value_to_unwrap.read()

    def path_exists_in_memory(self, path_to_check):
        return self.get_obj_at_path(path_to_check) is not None
    ...
    def teardown(self):
        ...
        all_paths = self.__flatten(self.swarm_memory_contents)
    ...
    def swarm_memory_interface_handle_request_connection_message(self, message):
        ...
        self.executor_interface.send_directed_message(
            ...
            {"SWARM_MEMORY_REF": self.get_data_to_holder_id_map(self.swarm_memory_contents)}
        )
    ...
    def swarm_memory_interface_handle_sync_swarm_memory_message(self, message):
        ...
        flattend_ref = self.__flatten(swarm_memory_ref)
        for path, holders in flattend_ref.items():
            if (not self.path_exists_in_memory(path)) or (not isinstance(self.get_obj_at_path(path), LocalSwarmMemoryEntry)):
                self.initialize_path_in_memory(path)
            entry = self.get_obj_at_path(path)
            for holder in holders:
                entry.add_holder_id(holder.get_holder_id(), holder.get_time_added())
            self.check_path_watchers(path)

        final_swarm_memory_ref = self.__flatten(self.get_data_to_holder_id_map(self.swarm_memory_contents))
    ...
    def get_data_to_holder_id_map(self, map_to_convert):
        if isinstance(map_to_convert, dict):
            ...
            for key, value in map_to_convert.items():
                if isinstance(value, dict):
                    ret_dict[key] = self.get_data_to_holder_id_map(value)
                else:
    ...
    def swarm_memory_interface_handle_request_new_holder_message(self, message):
        ...
        if (not self.local_memory_is_full()) and (not self.has_path_saved_locally(path_to_write)):
            if (not self.path_exists_in_memory(path_to_write)) or \
                    (not isinstance(self.get_obj_at_path(path_to_write), LocalSwarmMemoryEntry)):
                self.initialize_path_in_memory(path_to_write)
            ...
            local_entry = self.get_obj_at_path(path_to_write)
    ...
    def swarm_memory_interface_handle_bot_teardown_message(self, message):
        ...
        final_swarm_memory_ref = self.__flatten(self.swarm_memory_contents)
        for path in final_swarm_memory_ref:
            self.get_obj_at_path(path).remove_holder_id(bot_to_remove)
    ...
    def get_local_swarm_memory_contents(self):
        ...
        for path in all_paths:
            if self.has_path_saved_locally(path):
                local_contents[path] = self.get_obj_at_path(path).get_inner_value()
